...
bool LinkedList<ItemType>::insert(int newPosition, const ItemType& newEntry) throw(PrecondViolatedExcep)
{
   ...
   if (ableToInsert)
   {
	...
	throw PrecondViolatedExcep("You can't add an item there.");
   
   return ableToInsert;
}  // end insert

template<class ItemType>
bool LinkedList<ItemType>::remove(int position) throw(PrecondViolatedExcep)
{
   bool ableToRemove = (position >= 1) && (position <= itemCount) && (itemCount != 0);
   if (ableToRemove)
   {
	...
	throw PrecondViolatedExcep("There is no item there to remove ");
   return ableToRemove;
}  // end remove
